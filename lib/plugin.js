...
export default ({ app }) => {
  app.seomaticMeta = async function nuxtSeomaticMeta({ fullPath }, id = null) {
    ...
    consola.info(`Apollo: ${app.$apollo}`)
    consola.info(`Apollo Provider: ${app.apolloProvider}`)
    consola.info(`Apollo Client: ${app.apolloProvider.defaultClient}`)
    ...
    const { data } = await app.$apollo.query({
      ...
      query: gql`
          {
            seomatic(uri: $uri, asArray: $asArray, siteId: $siteId) {
              metaTitleContainer
              metaTagContainer
              metaLinkContainer
              metaScriptContainer
